...
function handle_call(call, exits){
    /*
        ...
        - Checks parameters that are functions as well. Assumes unique 
        callbacks (i.e. each parameter function gets called every time.)
    */
...
function sort(parent, exiting_functions, filetext){
    /*
        ...
        they exit. Nodes that exit are in the 'exit' property; nodes that do 
        not are in the 'non_exits' property.
    */
...
function check_functions(program, exits){
    /*
        Parses all function declarations in the program scope, returning those 
        that contain an exit call. Used in conjunction with the function_names 
        property of is_exit to handle the "wrapped exit call" case.
    */
    var exiting = [];
...
function digest_command(node, exiting_functions, params){
    if (!(params)){
        params = {};
    }
    ...
    if (node.type === 'ExpressionStatement'){
        str += digest_command(node.expression, exiting_functions, params).cmd
        ...
        if (callee.name){
            ...
            } else if (callee.object.callee.name){
                str += callee.object.callee.name;
            }
            else{
                str += digest_command(callee.object, exiting_functions, params).cmd;
            }
            str += "." + callee.property.name
            var checked = check_expr(node, exiting_functions);
            if (checked.exits !== undefined && checked.params != ''){
                var name = callee.property.name;
                params[name] = checked.exits;
            }
        }
...
function check_expr(expression, exiting_functions){
    var names = ['then', 'fail']
    ...
    for (var i = 0; i < names.length; i++) {
        if (callee.property.name === names[i]){
            for (var j = 0; j < expression.arguments.length; j++) {
                ...
                for (var k = 0; k < exiting_functions.length; k++) {
                    if (arg.name === exiting_functions[k]){
                        exits = true;
                    }
                }
            }
