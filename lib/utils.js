...
function is_exit(node, function_names){
  ...
  */
  ...
  switch(node.type){
    ...
    case consts.return:
      return is_exit(node.argument, exits);
...
function has_exit(node, function_names){
  ...
  */
  var actual_body = Array.isArray(node) ? node : [];
  if (node && node.body){
...
function nodes_equal(node1, node2){
  /* 
    Checks whether two nodes are equal by comparing the range of code each 
    node covers.
...
function get_all_functions(expr){
  /*
    ...
    ex. for `one().two().three().(...)` returns [`one`, `two`, `three`, ...]
      as esprima nodes, not strings
...
function test_if(if_statement, err, exits){
  /*
    ...
    Used in callbacks to determine error handling, but stored here because it
    doesn't actually have anything to do with errors or callbacks -- it finds
    (and checks) any kind of "if var not null" statements. `callback.js` does
    the heavy lifting of telling it *what* to check.
    ...
    @return: an object with the following properties:
      ...
      `err_exits`: whether the node exits, if `err_caught` is true
  */
