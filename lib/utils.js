...
function digest_command(node, exiting_functions){
    var str = "";
    if (node.type === 'ExpressionStatement'){
        str += digest_command(node.expression, exiting_functions)
        ...
        if (callee.name){
            ...
            if (callee.object.name){
                ...
                str += digest_command(callee.object, exiting_functions);
            }
            str += "." + callee.property.name
            var checked = check_params(node, exiting_functions);
            if (checked.exits !== undefined && checked.params != ''){
                if (checked.exits){
                    console.log(callee.property.name, "exits");
                } else{
                    console.log(callee.property.name, "doesn't exit");
                }
            }
        }
...
function check_params(expression, exiting_functions){
    var names = ['then', 'fail']
