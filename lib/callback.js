...
function reduce(all_results){
   /*
      ...
      @return: an object with the following properties:
         `need_exits`: an array of objects with the following properties:
            `success_found`: a boolean of whether there was an on success call
   ...
   for (var i = 0; i < all_results.length; i++) {
      if (something_exited(all_results[i])){
         ...
         if (!both_exited(all_results[i])){
            ...
            // remember that there was a fully exiting callback
            exit_count++;
         }
      }
   }

   var always_exits = !!exit_count && exit_count === to_return.length;

   for (i = 0; i < to_return.length; i++) {
      // shuffle around important information and lose the useless stuff
   ...
   */
   if (result.always_exits){
      ...
      for (var i = 0; i < result.need_exits.length; i++) {
         if (!(both_exited(result.need_exits[i]))){
            if (str) str += "\n\n";
            str += "For callback beginning on line " + result.need_exits[i].loc.start.line + ":";
            if (result.need_exits[i].success_found){
               if (result.need_exits[i].success_exits){
            ...
            if (result.need_exits[i].error_found){
               if (result.need_exits[i].error_exits){
                  str += "\n  - Caught errors exit.";
               } else {
