...
function walk_node(node, syntax, exit_funcs, code){
    /*
            ...
            if (childNode !== null){
                if (Array.isArray(childNode)){
                    for (var j = 0; j < childNode.length; j += 1){
                        ...
                        if (childElement){
                            var sorted = utils.sort(childElement, exit_funcs, code);
                            if (sorted.exits.length || sorted.non_exits.length){
                                all_nodes.push(sorted);
                            }
                            children = children.concat(walk_node(childElement, syntax, exit_funcs, code));
                        }
                    }
                } else {
                    var sorted = utils.sort(childNode, exit_funcs, code);
                    if (sorted.exits.length || sorted.non_exits.length){
                        all_nodes.push(sorted);
                    }
                    children = children.concat(walk_node(childNode, syntax, childNode, code));
                }
            }
        }
    }

    var my_nodes = all_nodes;

    for (var i = 0; i < children.length; i++) {
        all_nodes = all_nodes.concat(children[i].all);
    }
    return {
        'all': all_nodes,
        'children': children,
