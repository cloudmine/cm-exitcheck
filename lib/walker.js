...
function walk_node(node, code, syntax, exit_funcs, parser){
   /*
         ...
         if (childNode !== null){
            if (Array.isArray(childNode)){
               for (var j = 0; j < childNode.length; j += 1){
                  ...
                  if (childElement){
                     var parsed_element = parser(childElement, exit_funcs, code);
                     if (parsed_element){
                        all_nodes.push(parsed_element);
                     }
                     all_nodes = all_nodes.concat(walk_node(childElement, code, syntax, exit_funcs, parser));
                  }
               }
            } else {
               var parsed_child = parser(childNode, exit_funcs, code);
               if (parsed_child){
                  all_nodes.push(parsed_child);
               }
               all_nodes = all_nodes.concat(walk_node(childNode, code, syntax, childNode, parser));
            }
         }
      }
   }
