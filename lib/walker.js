...
function walk_node(node, syntax, exit_funcs){
    /*
            ...
            if (childNode !== null){
                if (Array.isArray(childNode)){
                    for (var j = 0; j < childNode.length; j += 1){
                        ...
                        if (childElement){
                            var sorted = utils.sort(childElement, exit_funcs);
                            if (sorted.exits.length || sorted.non_exits.length){
                                all_nodes = all_nodes.concat(sorted);
                            }
                            all_nodes = all_nodes.concat(walk_node(childElement, syntax, exit_funcs));
                        }
                    }
                } else {
                    var sorted = utils.sort(childNode, exit_funcs);
                    if (sorted.exits.length || sorted.non_exits.length){
                        all_nodes = all_nodes.concat(sorted);
                    }
                    all_nodes = all_nodes.concat(walk_node(childNode, syntax, childNode));
                }
            }
        }
    }
