...
function adjust_lines(single_result){
  /*
   * Adjusts line numbers to be zero-indexed.
   * Esprima starts at 1, but for some of us and our code that's unacceptable.
   *
   * @param json_result: an array of objects with at least 'line' property
   *
   * @return: a json result that's unchanged except to reduce each line number by 1
   */

  for (var i = 0; i < single_result.length; i++){
    single_result[i].line--;
  }
}
...
function run_esprima(code, options){
  ...
  */
  ...
  if (options && options.zero_index){
    // maybe someone wants a zero-indexed string
    adjust_lines(conditionals.need_exits);
    adjust_lines(promises.need_exits);
    adjust_lines(callbacks.need_exits);
  }
