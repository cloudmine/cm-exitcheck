...
function extract(tree){
   /*
      ...
      @param tree: a list of objects each with an 'exits' and 'non_exits' 
...
function generate_output_string(exits, non_exits, global_exits, promises){
   /*
      Generates final program output either as a readable string.

      @param exits: a list of esprima nodes which exit
      @param non_exits: a list of esprima nodes related to previous nodes which 
         do not exit
      @param global_exits: a list of esprima nodes of program scope which exit
      @param promises: a list of fcn calls that return promises, which have 
      ...
      @return: a string explaining problems or lack thereof.
         ...
         parsing output for success/failure condition is more complicated
         - strings which begin with neither of the above never exit
   */
   if (promises.length){
      var str = "";
...
function run_esprima(code, options){
   ...
   */
   ...
   for (var i = 0; i < parsed.body.length; i++){
      ...
      if (callback.test(parsed.body[i], exit_funcs.concat(['exit']))){
         exits.push(parsed.body[i]);
      }
   }
   ...
   return generate_output_string(exits, non_exits, global_exits, promises);
}
