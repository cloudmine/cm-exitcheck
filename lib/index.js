...
function run_esprima(code, options){
    ...
    for (var i = 0; i < parsed.body.length; i++){
        ...
        var cmd = utils.digest_command(parsed.body[i], exit_funcs);
        if (cmd){
            console.log(cmd);
        }
    }
    ...
    if (bool){
        if (global_exits.length){
            ...
            if (!(non_exits.length) && exits.length){
                // everything exits
                return false;
            }
            return false;
    ...
    if (global_exits.length){
        ...
        if (non_exits.length){
            ...
            if (exits.length){
                ...
                return "Your code never exits. Adding the line \'exit()\' to the end of your code will prevent the process from hanging."
            }
        }
    }
}
...
var bnc_folder = './test/fixtures/bnc/';
var filenames = fs.readdirSync(bnc_folder);

for (var i = 0; i < filenames.length; i++) {
    var filename = filenames[i]
    console.log("\nfor", filename);
    var result = read_file(bnc_folder + filenames[i], {
        'json': true
    });
}
