...
function run_esprima(code){
    ...
    for (var i = 0; i < tree.length; i++) {
        for (var j = 0; j < tree[i].exits.length; j++) {
            var str = tree[i].exits[j].condition;
            var start = tree[i].exits[j].loc.start.line;
            var end = tree[i].exits[j].loc.end.line;
            str += " (lines " + start + "-" + end + ")"
            exits.push(str);
        }
        for (var j = 0; j < tree[i].non_exits.length; j++) {
            var str = tree[i].non_exits[j].condition;
            var start = tree[i].non_exits[j].loc.start.line;
            var end = tree[i].non_exits[j].loc.end.line;
            str += " (lines " + start + "-" + end + ")";
            non_exits.push(str);
        }
    }
