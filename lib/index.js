...
function callback_string(callbacks){
   ...
   */
   ...
   for (var i = 0; i < callbacks.length; i++) {
      ...
      for (var j = callbacks[i].length - 1; j >= 0; j--) {
         var callback = callbacks[i][j];
         if (callback.exits){
            last_exit_index = j;
            break;
...
if (process.argv.length > 2){
   ...
   var path_match = /cm-exitcheck\/lib\/index\.js$/.exec(args[1]);
   if (args[0] === 'node' && (path_match || args[1] === 'lib/index.js')){
      // loop through actual args
      for (var i = 2; i < args.length; i++) {
         // console.log(JSON.stringify(read_file(args[i], {
         //    'json': true
         // }), null, 3));
