...
function generate_output_string(global_exits, conditionals, promises, callbacks){
  /*
  ...
  */
  if (global_exits.length){
    ...
    if (promises.need_exits.length){
      if (str !== ""){
        // give a little space between messages
        str += "\n\n";
      }
...
function run_esprima(code, options){
  ...
  */
  ...
  // remove extraneous information from global exit nodes (esprima nonsense)
  ...
  if (options && options.json){  
    ...
    return generate_output_string(global_exits, conditionals, promises, callbacks);  
  }
}
